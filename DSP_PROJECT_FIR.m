...
% FIR Hamming window
b_fir = fir1(fir_order, Wn_fir, 'low', hamming(fir_order+1));
a_fir = 1;

% FIR FILTER FREQUENCY RESPONSE
figure('Name','FIR Filter Frequency Response','Color','w');
freqz(b_fir, a_fir, 2048, Fs);
title('Frequency Response of the FIR Low-Pass Filter');


%% FIR FILTER APPLICATION

x_filt_fir = filtfilt(b_fir, a_fir, x_sensor);
y_filt_fir = filtfilt(b_fir, a_fir, y_sensor);
z_filt_fir = filtfilt(b_fir, a_fir, z_sensor);

% TIME-DOMAIN COMPARISON
figure('Name','FIR Filter - Time Domain Comparison','Color','w');
subplot(3,1,1);
plot(t, x, 'g', 'LineWidth', 2); hold on;
plot(t, x_sensor, 'r', 'LineWidth', 2);
plot(t, x_filt_fir, 'b', 'LineWidth', 2);
grid on;
xlabel('Time (s)');
ylabel('x (m)');
title('X Position');
legend('Clean x','Sensor x','Filtered x (FIR)');
ylim([-2.8 2.8]);

subplot(3,1,2);
plot(t, y, 'g', 'LineWidth', 2); hold on;
plot(t, y_sensor, 'r', 'LineWidth', 2);
plot(t, y_filt_fir, 'b', 'LineWidth', 2);
grid on;
xlabel('Time (s)');
ylabel('y (m)');
title('Y Position');
legend('Clean y','Sensor y','Filtered y (FIR)');
ylim([-2.8 2.8]);

subplot(3,1,3);
plot(t, z, 'g', 'LineWidth', 2); hold on;
plot(t, z_sensor, 'r', 'LineWidth', 2);
plot(t, z_filt_fir, 'b', 'LineWidth', 2);
grid on;
xlabel('Time (s)');
ylabel('z (m)');
title('Z Position');
legend('Clean z','Sensor z','Filtered z (FIR)');


%% FINAL SPECTRAL COMPARISON WITH FIR

% Remove DC component
x_filt_fir_d = x_filt_fir - mean(x_filt_fir);
y_filt_fir_d = y_filt_fir - mean(y_filt_fir);
z_filt_fir_d = z_filt_fir - mean(z_filt_fir);

% FFT of filtered signals
X_filt_fir = fft(x_filt_fir_d);
Y_filt_fir = fft(y_filt_fir_d);
Z_filt_fir = fft(z_filt_fir_d);

% One-sided magnitude spectrum
X_filt_fir_mag = 2*abs(X_filt_fir(1:floor(N/2)+1))/N;
Y_filt_fir_mag = 2*abs(Y_filt_fir(1:floor(N/2)+1))/N;
Z_filt_fir_mag = 2*abs(Z_filt_fir(1:floor(N/2)+1))/N;

% Correct DC bin
X_filt_fir_mag(1) = X_filt_fir_mag(1)/2;
Y_filt_fir_mag(1) = Y_filt_fir_mag(1)/2;
Z_filt_fir_mag(1) = Z_filt_fir_mag(1)/2;

% FREQUENCY-DOMAIN COMPARISON
figure('Name','FIR Filter - Frequency Domain Comparison','Color','w');
subplot(3,1,1);
plot(f, X_mag, 'g', 'LineWidth', 2); hold on;
plot(f, X_sensor_mag, 'r', 'LineWidth', 2);
plot(f, X_filt_fir_mag, 'b', 'LineWidth', 2);
grid on;
xlabel('Frequency (Hz)');
ylabel('|X(f)|');
title('FFT of X Position');
...
plot(f, Y_sensor_mag, 'r', 'LineWidth', 2);
plot(f, Y_filt_fir_mag, 'b', 'LineWidth', 2);
grid on;
xlabel('Frequency (Hz)');
ylabel('|Y(f)|');
title('FFT of Y Position');
...
plot(f, Z_sensor_mag, 'r', 'LineWidth', 2);
plot(f, Z_filt_fir_mag, 'b', 'LineWidth', 2);
grid on;
xlabel('Frequency (Hz)');
ylabel('|Z(f)|');
title('FFT of Z Position');
